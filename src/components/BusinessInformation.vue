            ...
            <div class="col-lg-10 col-md-9 col-sm-12">
               <div class="login container text-center">
                  <div class="m-auto tricolor-logo">
                     ...
                     <h1 class="text-uppercase page-name mt-3 text-nowrap">Business information</h1>
                  </div>
                     ...
                     <validation-observer ref="observer" v-slot="{ handleSubmit }">
                        <b-form @submit.stop.prevent="handleSubmit(add)">
                           <validation-observer ref="observer" v-slot="{ handleSubmit }">
                              <b-form @submit.stop.prevent="handleSubmit(add)">
                                 <div class="row">
                                    <div class="col-lg-8 col-md-8 col-12 m-auto clientInfoField">
                                       ...
                                       <!-- <BusinessName :getBusinessName="getBusiness"  class="validation-component"/> -->
                                        <b-form-group id="example-input-group-2" label="Email" label-for="example-input-2"   class="shipping-label container form-labels email-label pl-2 text-start">
                                          <div class="row" >
                                             <div class="col-lg-11 col-md-11 col-11 pr-0 pt-0"><Email :getEmail="getUserEmail"/></div>
                              ...
                              <Button type="submit" variant="primary" class="text-uppercase create-btn next mb-2 clientNextBtn">Next</Button>
                        </b-form>
                     </validation-observer>
                  </div>
               </div>
    ...
    data(){
      ...
      //   },
       ...
       load(){
     		axios({
     			method: 'get',
     			url: 'http://localhost:3000/businessInformation/'
     		})
     		.then(res => {
     			this.businessInformation = res.data 
     		})
     		.catch ((err) => {
     			this.alertError()
     		})
       ...
       edit(user){ 
         ...
         this.form.businessName = user.businessName
       },
       update(form){
       	axios({
     		  method: 'put',
     		  url: 'http://localhost:3000/businessInformation/' + form.id,
     		  data: {id: form.id, username: this.form.username, email:this.form.email, password:this.form.password, confirmPassword:this.form.confirmPassword, businessName: this.form.businessName}
   		  })
         .then(res => {
         //   this.load()
           this.form.id = ''
       ...
       del(user){
          if (confirm('Are you sure you want to delete the data?')) {
             axios({
                           method: 'delete',
                           url: 'http://localhost:3000/businessInformation/' + user.id,	
                        })
                        .then(res =>{
                           // this.load()
                           this.form.id = ''
                           ...
                           this.form.businessName =''
                           this.updateSubmit = false
                           let index = this.businessInformation.indexOf(this.form.username)
                           this.businessInformation.splice(index,1)
                           this.alertSuccess()
                        });
